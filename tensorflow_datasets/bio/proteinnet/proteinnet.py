...
import os
from typing import Any, Dict, Sequence, Union, Generator, Tuple, Optional, List
...
Example = Dict[str, Union[int, str, np.ndarray, List]]
ExampleGenerator = Generator[Tuple[str, Example], None, None]
...
_PROTEINNET_URL = 'https://sharehost.hms.harvard.edu/sysbio/alquraishi/proteinnet/human_readable/'
_PROTEINNET_FILES = {
    'casp7': 'casp7.tar.gz',
    'casp8': 'casp8.tar.gz',
    'casp9': 'casp9.tar.gz',
...
_PROTEINNET_TRAIN_THRESHOLDS = [30, 50, 70, 90, 95, 100]
_AMINOACIDS = ['A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'P',
               'Q', 'R', 'S', 'T', 'V', 'W', 'Y']
...
def _read_entry(fin: Any) -> Optional[Tuple[str, Example]]:
  ...
  # Check structure.
  assert lines[0] == '[ID]'
  assert lines[2] == '[PRIMARY]'
  assert lines[4] == '[EVOLUTIONARY]'
  assert lines[26] == '[TERTIARY]'
  assert lines[30] == '[MASK]'
...
class ProteinNet(tfds.core.GeneratorBasedBuilder):
  ...
  URL = _PROTEINNET_URL
  FILES = _PROTEINNET_FILES
  THRESHOLDS = _PROTEINNET_TRAIN_THRESHOLDS
    ...
    return tfds.core.DatasetInfo(
        ...
        features=tfds.features.FeaturesDict({
            ...
            'evolutionary': tfds.features.Tensor(
                shape=(None, 21), dtype=tf.float32),
            'tertiary': tfds.features.Tensor(shape=(None, 3), dtype=tf.float32),
            'mask': tfds.features.Tensor(shape=(None,), dtype=tf.bool),
  ...
  def _split_generators(
    ...
    ) -> Dict[Union[str, tfds.Split], ExampleGenerator]:
    ...
    train_splits = {
        'train_%d' % threshold: self._generate_examples(
            os.path.join(path, name, 'training_%d' % threshold))
    ...
    splits = {}
    splits.update(train_splits)
    splits.update(validation_split)
    splits.update(test_split)
    return splits

  def _generate_examples(self, filename: str) -> ExampleGenerator:
    """Yields examples."""

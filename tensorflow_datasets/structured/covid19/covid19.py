...
"""TODO(alcastano): DO NOT SUBMIT without one-line documentation for covid19_2.

TODO(alcastano): DO NOT SUBMIT without a detailed description of covid19_2.
"""
...
# TODO(covid19): Markdown description  that will appear on the catalog page.
_DESCRIPTION = """
...
class Covid19(tfds.core.GeneratorBasedBuilder):
  ...
  def _info(self) -> tfds.core.DatasetInfo:
    ...
    # TODO(covid19): Specifies the tfds.core.DatasetInfo object
  ...
  def _split_generators(self, dl_manager: tfds.download.DownloadManager):
    ...
    return {
        tfds.Split.TRAIN:
            self._generate_examples(dl_manager.iter_archive(archive_path))
    }

  def _generate_examples(self, file_handler):
    """Yields examples."""
    pd = tfds.core.lazy_imports.pandas
    beam = tfds.core.lazy_imports.apache_beam

    def cast_according_to_column(k, v, features):
      if features[k].dtype == tf.string and ((type(v) is float) or
                                             (type(v) is int)):
        return str(v)
      return v
    _, file = next(file_handler)
    ...
    def f(pair, columns):
      index,row = pair
      example = {
          k: cast_according_to_column(k, v, features)
          for k, v in zip(columns, row.values)
      }
      return index, example
    return (
        beam.Create(list(df.iterrows()))
        | beam.Map(lambda x: f(x,columns))
    )
